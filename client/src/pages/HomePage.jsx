...
export default function HomePage() {
  ...
  function loadWatched() {
    ...
    api.get('/movies/watched')
      .then((r) => {
        setWatchedMovies(r.data.movies)
        setWatchedLoaded(true)
      })
      .catch(() => toast.error('Failed to load watched movies'))
      .finally(() => setWatchedLoading(false))
  }

  function handleSwitchTab(newTab) {
    setTab(newTab)
    if (newTab === 'watched' && !watchedLoaded) loadWatched()
  }
  ...
  const existingTmdbIds = useMemo(
    () => new Set([...movies, ...watchedMovies].filter((m) => m.tmdbId).map((m) => m.tmdbId)),
        ...
        <div className="flex gap-1 bg-zinc-900 border border-zinc-800 rounded-xl p-1 w-fit mb-6">
          ...
          >
            ...
            {watchedMovies.length > 0 && (
              <span className="ml-1.5 text-xs opacity-70">({watchedMovies.length})</span>
            )}
          </button>
        </div>
        ...
        {tab === 'watched' && (
          <>
            {watchedLoading ? (
              ...
              <div className="text-center py-24">
                <div className="text-5xl mb-4">📺</div>
                <p className="text-zinc-400 text-lg">No movies watched yet.</p>
                <p className="text-zinc-600 text-sm mt-2">
                  Use the eye icon on a movie card to mark it as watched.
                </p>
              </div>
            ) : (
              <>
